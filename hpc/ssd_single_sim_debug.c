...
int main(int argc, char *argv[]) {
	...
	while (current_time<INTRODUCE_AFTER && n_event < MAX_N_EVENTS) {
		...
		if (current_time>=recording_time) {
			...
			// Check if mutant counts went out of bounds
			int** true_mutant_counts = malloc(CELLS * sizeof(int*));
			for (int k=0; k<CELLS; ++k) {
				true_mutant_counts[k] = calloc(2*(mutant_counts[0][0]+1), sizeof(int));
			}
			get_mutant_counts_wildtype(true_mutant_counts, wildtype_state, wildtype_populations);
			if (!(identical_mutant_counts(mutant_counts, true_mutant_counts))) {
				printf("n_event = %d\n", n_event);
				exit(99);
			}
	...
	while (current_time<SIM_LENGTH && n_event<MAX_N_EVENTS) {
		...
		if (current_time>=recording_time){
			...
			get_mutant_counts(true_mutant_counts, wildtype_state, ssd_state, wildtype_populations, ssd_populations);
			if (!(identical_mutant_counts(mutant_counts, true_mutant_counts))) {
				printf("n_event = %d\n", n_event);
				exit(99);
			}
