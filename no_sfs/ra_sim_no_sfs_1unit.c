...
double propensity_update(double propensity[4], unsigned int state[2], double degradation_rate, double nucleus_control_factor, double target_population, double replicative_advantage) {
	...
	propensity[3] = (replication_rate + replicative_advantage) * m;
...
int main(int argc, char *argv[]) {
	...
	double replicative_advantage = 2.5e-1;
	...
	char filename[50];
	sprintf(filename, "ra_sim_no_sfs_1unit_%d.txt", seed);
	...
	for (unsigned int sim=0; sim<1000; ++sim) {
		...
		while (current_time<final_time) {
			propensity_sum = propensity_update(propensity, state, degradation_rate, nucleus_control_factor, target_population, replicative_advantage);
			...
			gillespie_event(rng, propensity, state, degradation_rate, nucleus_control_factor, target_population, replicative_advantage);
